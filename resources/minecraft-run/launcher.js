...
class MCLCore extends EventEmitter {
  async launch (options) {
    try {
      ...
      // Create directory for pack
      fs.mkdir(this.options.root, {recursive: true}, () => {})
      ...
      if (!java.run) {
        ...
        this.emit('status', {message: "Не получилось запустить игру", type: "error"})
        this.emit('close', 1)
        return null
      }
      ...
      this.emit('status', {message: "Собираем пакеты", type: "info"})
      for (const download of options.downloads) {
        if (!fs.existsSync(path.join(options.root, download["to"], download["name"])) || download["forgeForceUpdate"]) {
          await this.handler.downloadAsync(download["url"], path.join(options.root, download["to"]), download["name"], true, "customs")
          if (download["unzip"]) {
            const zip = new Zip(path.join(options.root, download["to"], download["name"]))
            zip.extractAllTo(path.join(options.root, download["to"]));
            fs.unlinkSync(path.join(options.root, download["to"], download["name"]));
          }
        }
      }
      ...
      if (this.options.installer) {
        ...
        if (!this.options.version.custom && code === 0) {
          ...
          this.emit('status', {message: "Установщик запущен", type: "info"})
        }
        this.emit('debug', `[MCLC]: Installer closed with code ${code}`)
        this.emit('status', {message: "Установщик выключен с кодом "+code, type: "error"})
      }
      ...
      if (!fs.existsSync(mcPath)) {
        ...
        this.emit('status', {message: "Скачиваем версию майнкрафта", type: "info"})
      ...
      this.emit('status', {message: "Используем сепаратор", type: "info"})
      ...
      this.emit('status', {message: "Скачиваем ассеты", type: "info"})
      await this.handler.getAssets()

      // Download modpack
      if (this.options.packLink) {
        this.emit('status', {message: "Скачиваем сборку", type: "info"})
        fs.rmSync(path.join(options.root, "temp", "pack.zip"), { recursive: true, force: true });
        await this.handler.downloadAsync(this.options.packLink, path.join(options.root, "temp"), "pack.zip", true, "customs")
        if (fs.existsSync(path.join(options.root, "temp", "pack.zip"))) {
          fs.rmSync(path.join(options.root, "temp", "pack"), { recursive: true, force: true });
          const zip = new Zip(path.join(options.root, "temp", "pack.zip"))
          zip.extractAllTo(path.join(options.root, "temp", "pack"));

          // Move overrides
          if (fs.existsSync(path.join(options.root, "temp", "pack", "overrides"))) {
            mv(path.join(options.root, "temp", "pack", "overrides"), options.root, {clobber: false}, (err) => {
              console.log(err)
            })
          }
        }
      ...
      this.emit('status', {message: "Запускаем игру!", type: "info"})
  ...
  async getModifyJson () {
    ...
    if (this.options.forge) {
      this.options.forge = path.join(this.options.root, "forge", "1.18.2", "installer.jar")
      ...
      modifyJson = JSON.parse(fs.readFileSync(path.join(this.options.root, 'versions', this.options.version.custom, `${this.options.version.custom}.json`), {encoding: 'utf8'}))
    }
  ...
  startMinecraft (launchArguments) {
    console.log(this.options.javaPath ? this.options.javaPath : 'java', launchArguments,
    ...
    this.emit('status', {message: "Игра запущена!", type: "success"})
    return minecraft
  }
}
